...
bool Level_2_Scene::init()
{
    ...
    GameManager::getInstance().selectLevel(GameManager::Level::LEVEL_2);
    // Tạo âm thanh
    SoundManager::playBackground(AudioPaths::LEVEL2_BACKGROUND, true);
    // Tạo map
    auto map = LevelManager::loadLevel(Level::LEVEL_2);
    map->setPosition(Vec2(0, 0));
    this->addChild(map);

    // Chỉnh lại thông số của 1 vài object
    auto saw1 = static_cast<Saw*>(map->getChildByName("Saw1"));
    saw1->setAmplitude(576.0f);
    saw1->setFrequency(0.1f);
    saw1->activateTrap();

    auto saw2 = static_cast<Saw*>(map->getChildByName("Saw2"));
    saw2->setAmplitude(-576.0f);
    saw2->setFrequency(0.1f);
    saw2->activateTrap();

    // Tạo player
    auto player1 = GameObjectManager::createPlayer();
    player1->setPosition(Vec2(240, 480));
...
void Level_2_Scene::update(float delta)
{
    switch (_gameState)
    {
        case GameState::init:
...
Level_2_Scene::Level_2_Scene()
{
    AXLOGD("Scene: Level_2 ctor");
}

Level_2_Scene::~Level_2_Scene()
{
    AXLOGD("~Scene: Level_2 dtor");
}