...
bool Level_4_Scene::init()
{
    ...
    GameManager::getInstance().selectLevel(GameManager::Level::LEVEL_4);
    // Tạo âm thanh
    SoundManager::playBackground(AudioPaths::LEVEL4_BACKGROUND, true);
    // Tạo map
    auto map = LevelManager::loadLevel(Level::LEVEL_4);
    map->setPosition(Vec2(0, 0));
    this->addChild(map);

    // Chỉnh lại thông số của 1 vài object
    auto saw1 = static_cast<Saw*>(map->getChildByName("Saw1"));
    saw1->setAmplitude(-415.0f);
    saw1->setFrequency(0.15f);
    saw1->activateTrap();

    auto saw2 = static_cast<Saw*>(map->getChildByName("Saw2"));
    saw2->setAmplitude(-560.0f);
    saw2->setFrequency(0.1f);
    saw2->activateTrap();

    auto saw3 = static_cast<Saw*>(map->getChildByName("Saw3"));
    saw3->setAmplitude(-560.0f);
    saw3->setFrequency(0.1f);
    saw3->activateTrap();

    auto rockHead1 = static_cast<RockHead*>(map->getChildByName("RockHead1"));
    rockHead1->setAmplitude(528.0f);
    rockHead1->setFrequency(0.08f);
    rockHead1->activateTrap("horizontal");

    auto rockHead2 = static_cast<RockHead*>(map->getChildByName("RockHead2"));
    rockHead2->setAmplitude(384.0f);
    rockHead2->setFrequency(0.2f);
    rockHead2->activateTrap("horizontal");

    auto rockHead3 = static_cast<RockHead*>(map->getChildByName("RockHead3"));
    rockHead3->setAmplitude(-175.0f);
    rockHead3->setFrequency(0.07f);
    rockHead3->activateTrap("horizontal");

    auto rockHead4 = static_cast<RockHead*>(map->getChildByName("RockHead4"));
    rockHead4->setAmplitude(128.0f);
    rockHead4->setFrequency(0.12f);
    rockHead4->activateTrap("vertical");

    // Tạo player
    auto player1 = GameObjectManager::createPlayer();
    player1->setPosition(Vec2(40, 100));
...
void Level_4_Scene::update(float delta)
{
    switch (_gameState)
    {
        case GameState::init:
...
{
    AXLOGD("Scene: Level_4 ctor");
}

Level_4_Scene::~Level_4_Scene()
{
    AXLOGD("~Scene: Level_4 dtor");
}