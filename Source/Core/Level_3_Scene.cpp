...
bool Level_3_Scene::init()
{
    ...
    // Tạo âm thanh
    SoundManager::playBackground(AudioPaths::LEVEL3_BACKGROUND, true);
    // Tạo map
    auto map = LevelManager::loadLevel(Level::LEVEL_3);
    map->setPosition(Vec2(0, 0));
    this->addChild(map);

    // Chỉnh lại thông số của 1 vài object
    auto saw1 = static_cast<Saw*>(map->getChildByName("Saw1"));
    saw1->setAmplitude(-580.0f);
    saw1->setFrequency(0.1f);
    saw1->activateTrap();

    auto saw2 = static_cast<Saw*>(map->getChildByName("Saw2"));
    saw2->setAmplitude(-580.0f);
    saw2->setFrequency(0.1f);
    saw2->activateTrap();

    auto saw3 = static_cast<Saw*>(map->getChildByName("Saw3"));
    saw3->setAmplitude(-450.0f);
    saw3->setFrequency(0.1f);
    saw3->activateTrap();

    auto rockHead1 = static_cast<RockHead*>(map->getChildByName("RockHead"));
    rockHead1->setAmplitude(528.0f);
    rockHead1->setFrequency(0.2f);
...
void Level_3_Scene::update(float delta)
{
    switch (_gameState)
    {
        case GameState::init:
...
{
    AXLOGD("Scene: Level_3 ctor");
}

Level_3_Scene::~Level_3_Scene()
{
    AXLOGD("~Scene: Level_3 dtor");
}